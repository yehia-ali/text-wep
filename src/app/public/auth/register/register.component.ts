...
@Component({
  ...
  standalone: true,
  imports: [
    CommonModule, LanguageComponent, ReactiveFormsModule, TranslateModule,
    InputLabelComponent, InputErrorComponent, NgxIntlTelInputModule, MatRadioModule,
    FormsModule, ArabicNumbersPipe, SubmitButtonComponent, MatButtonModule,
    LogoComponent, InputPasswordComponent, AuthLayoutComponent
  ],
...
export class RegisterComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    private dataService: RegisterSharedDataService
  ) {
  ...
  ngOnInit(): void {
    const phone = this.dataService.getPhone();
    ...
    if (phone || email) {
      ...
      return; // Exit ngOnInit early if navigating away
    }
      ...
      this.source$ = this.activatedRoute.queryParams.subscribe((params) => {
        if (params['packageId']) {
          ...
          this.packages.map((item) => {
            if (item.id == params['packageId']) {
              this.numberOfEmployees = item.limitedNumberOfUser || 1;
  ...
  submit() {
    ...
    this.recaptchaV3Service.execute('importantAction').subscribe((token: any) => {
      data = {
        ...
        phoneNumber: this.phoneNumber || this.form.value.phone.e164Number.replace('+', '00'),
