...
@Component({
  ...
  template: `
      <div class="">
          <filter-label *ngIf="!hideLabel" key="task_type"></filter-label>
          <ng-select  [placeholder]="'task_type' | translate" (ngModelChange)="changeValue($event)" [(ngModel)]="selectedValue" [clearSearchOnAdd]="true" [closeOnSelect]="false"
                     [items]="list"
                     [multiple]="true" appendTo="body" bindLabel="name" bindValue="value" class="w-11r {{classes}}">
